...
@Controller
public class Db2DruidController {
    @Resource
    private DataSource druidDSDb2;
    ...
    @RequestMapping("/db2Druid")
    @ResponseBody
    public String test(HttpServletResponse resp){
        resp.setContentType("text/html;charset=UTF-8");
        StringBuffer stringBuffer=new StringBuffer();
        stringBuffer.append("<html><head><title></title></head><body><h2>Db2 Druid</h2>");
        String queryResult="<a style=\"color: red\">norecord</a>";
        try {
            Connection conn=druidDSDb2.getConnection();
