...
@Controller
public class PgDruidController {
    @Resource
    private DataSource druidDSPostgresql;
    ...
    @RequestMapping("/postgresqlDruid")
    @ResponseBody
    public String test(HttpServletResponse resp){
        resp.setContentType("text/html;charset=UTF-8");
        StringBuffer stringBuffer=new StringBuffer();
        stringBuffer.append("<html><head><title></title></head><body><h2>postgresql Druid</h2>");
        String queryResult = "<a style=\"color: red\">norecord</a>";

        try {
            Connection conn=druidDSPostgresql.getConnection();
